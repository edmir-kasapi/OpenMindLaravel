<x-layouts.app>
    ...
    <x-slot:title>
        {{ __('stores\stores.edit_store_admin') }}
    </x-slot:title>
    ...
    <section class="w-50 mx-auto">
            ...
            <div class="card-body">
                ...
                <form action="{{ route('admin.stores.update', $store->id) }}" method="post">
                    ...
                    <div class="input-group mb-3">
                        <span class="input-group-text @error('name') text-danger @enderror" id="basic-addon1">
                    ...
                    <div class="input-group mb-3">
                        <span class="input-group-text @error('country') text-danger @enderror">  {{ __('stores\stores.country') }} </span>
                    ...
                    <div class="input-group mb-3"> <!-- pattern="[0-9]{3}-[0-9]{3}-[0-9]{4}" -->
                        <span class="input-group-text @error('phone') text-error @enderror"
        ...
        @if (!$store->is_approved)
            <div class="card card-success card-outline mb-4">
                <div class="card-header">
                    <div class="card-title"> {{ __('stores\stores.unapproved_store_message') }}</div>
                </div>
                <div class="card-body">
                    ...
                    <form action="{{ route('admin.stores.approve', $store->id) }}" method="post">
                        ...
                        <div class="d-flex mt-3 gap-2">
                            <input type="submit" value=" {{ __('stores\stores.approve_store') }}" class="btn btn-success">
                        </div>
                    </form>
                </div>
            </div>
        ...
        @if ($store->is_approved)
            <div class="card card-danger card-outline mb-4">
                <div class="card-header">
                    <div class="card-title"> {{ __('stores\stores.approved_store_message') }}</div>
                </div>
                <div class="card-body">

                    <form action="{{ route('admin.stores.unapprove', $store->id) }}" method="post">
                        @csrf

                        <div class="d-flex mt-3 gap-2">
                            <input type="submit" value=" {{ __('stores\stores.unapprove_store') }}" class="btn btn-danger">
                        </div>
                    </form>
                </div>
            </div>
    ...
    <x-modals.danger-modal id="modal-disable-store" title="{{ __('stores\stores.disable_store') }}"
        ...
        form-id="admin-disable-store-form" submit-text="{{ __('stores\stores.disable') }}" method='DELETE' />

    <div id="admin-stores-config" data-delete-url="{{ url('admin/stores/delete') }}">
    </div>
