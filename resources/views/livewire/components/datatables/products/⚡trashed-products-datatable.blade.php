...
new class extends LiveDatatable {
    ...
    protected array $columnOrder = ['name', 'brand', 'type_id', 'price', 'stock_level', 'deleted_at', 'actions'];

    public ?string $productToRestore = null;
    public ?string $productToForceDelete = null;
    ...
    public function prepareRestoreProduct(string $id): void
    {
        $this->productToRestore = $id;
        $this->dispatch('show-modal', modalId: 'modal-restore-product');
    }

    public function prepareForceDeleteProduct(string $id): void
    {
        $this->productToForceDelete = $id;
        $this->dispatch('show-modal', modalId: 'modal-force-delete-product');
    }
    ...
    public function proceedRestore(): void
    {
        ...
        $product = Product::onlyTrashed()->find($this->productToRestore);

        if (!$product) {
            $this->productToRestore = null;
            $this->dispatch('show-error-message', message: 'Invalid product ID provided. Cannot Restore.');
            return;
        }

        $this->productService->restoreProduct($product);
        $this->dispatch('show-success-message', message: 'Product restored successfully');
        $this->goToPreviousPageIfEmpty($this->getData());
    }

    public function proceedForceDelete(): void
    {
        if (!$this->productToForceDelete) {
            return;
        }

        $product = Product::onlyTrashed()->find($this->productToForceDelete);

        if (!$product) {
            $this->productToForceDelete = null;
        ...
        $this->dispatch('show-success-message', message: 'Product permanently deleted');
        $this->goToPreviousPageIfEmpty($this->query());
    }

    public function cancelRestore(): void
    {
        $this->productToResore = null;
    }
    ...
    public function exportData(): ProductsExport
    {
        ...
        return $this->productService->exportTrashedProducts($query);
    }
    ...
    protected function columns(): array
    {
        return [
            ...
            'deleted_at' => [
                'label' => __('products/products-trashed.date_deleted'),
                'sortable' => true,
            ],
    ...
    {
        return Product::onlyTrashed()->with(['type', 'pictures']);
    }
...
@endphp <section class="w-75 mx-auto">
                        ...
                        <div class="card-header">
                            <div class="row g-2 align-items-center">
                                <div class="col-12 col-md-4">
                                    <h3 class="card-title">{{ __('products/products-trashed.deleted_products_list') }}
                                    </h3>
                                </div>
                                    ...
                                    <div class="d-flex flex-wrap justify-content-md-end gap-3">
                                        ...
                                        <a href="{{ route('admin.products') }}">
                                            <button type="button" class="btn btn-sm btn-primary" data-bs-toggle="modal"
                                                ...
                                                {{ __('products/products-trashed.active_products') }}
                                            </button>
                                        </a>
                                                            ...
                                                            @case('actions')
                                                                <x-datatable.buttons.button-layout>
                                                                    <x-datatable.buttons.action-button-mk2 type="success"
                                                                        icon="arrow-repeat"
                                                                        wire:click="prepareRestoreProduct({{ $product->id }})" />


                                                                    <x-datatable.buttons.action-button-mk2 type="danger"
                                                                        icon="clipboard2-x"
                                                                        wire:click="prepareForceDeleteProduct({{ $product->id }})" />
                                                                </x-datatable.buttons.button-layout>
                                                            @break
    ...
    <x-modals.confirm-modal-mk2 id="modal-restore-product" type="success"
        title="{{ __('modals/restore-product-modal.restore_product') }}"
        message="{{ __('modals/restore-product-modal.restore_product_confirmation') }}"
        submit-text="{{ __('modals/restore-product-modal.restore_product') }}" wire-confirm="proceedRestore()"
        wire-cancel="cancelRestore()" />

    <x-modals.confirm-modal-mk2 id="modal-force-delete-product" type="danger"
        title="{{ __('modals/force-delete-product-modal.force_delete_product') }}"
        message="{{ __('modals/force-delete-product-modal.force_delete_product_confirmation') }}"
        submit-text="{{ __('modals/force-delete-product-modal.force_delete') }}" wire-confirm="proceedForceDelete()"
        wire-cancel="cancelForceDelete()" />
